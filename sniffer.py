...
def receiveData(s):
	data = ''
	try:
		data = s.recvfrom(65565)	#Maximum size which we can capture is 65565
	except timeout:
		data = ''
	except:
...
version = version_IHL >> 4		#Shift it 4bits (version and IHL takes 8bits) to right
IHL = version_IHL & 0xF
TOS = unpackedData[1]
...
sourceAddress = socket.inet_ntoa(unpackedData[8])		#inet_ntoa - Convert a 32-bit packed IPv4 address (a string four characters in length) to its standard dotted-quad string representation
